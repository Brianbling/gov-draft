...
- **决议/公报题注（GB/T 9704 §7.3.1）**：决议/公报标题下须居中排"（会议名称 通过日期）"题注。`toMarkdown` 对 resolution/gazette 在标题容器后输出居中题注——决议取 issuer（会议全称）+ date（通过日期）排"（××会议 2026年×月×日通过）"；公报取 issuer（发布机关）+ date（发布日期）同理；date 缺失时只排会议名。prompt 增加决议/公报题注规范（决议把会议名称写进 title、issuer 填会议全称、date 填通过日期）。
- **纪要红色标志（GB/T 9704 §10.3）**：会议纪要标题即"××会议纪要"红色居中标志，置于版心上边缘下 20mm，不用 `#` 标题。`toMarkdown` 对 minutes 从标题剥"纪要"取会议名，渲染红色居中标志（小标宋、26pt、#e60012、spacing.before 20mm、字距 3pt）。
- **命令标题 prompt 规范（GB/T 9704 §10.2）**：命令（令）标题即"×××令"式标志（如"××市人民政府令"），prompt 明确禁止写"关于公布《×××》的命令"；`doc-type-spec` 新增 `ORDER_MISSING_LING_MARKER` 校验 issuingOrg 以"令"结尾（error 级）；命令文号规则放宽接受"第×号"占位（`/^第\s*(?:\d+|×+)\s*号/`）。
...
- **命令（令）红头丢失（批量审计发现）**：LLM 生成 order 时给了 `issuingOrg`（"××市人民政府令"），但 `toMarkdown` 的文件式红头分支按 `isFileRedHeadDocType()`（gongwen/communique）门控，order 的红头被整体丢弃 → 文号裸排居中、无红色标志。修复：`toMarkdown` 为 order 单独渲染红色居中标志（机关名全称+令，字号按机关名长度 42/36/30/26pt，spacing.before 20mm 对齐 GB/T 9704 §10.2 版心上边缘下 20mm，字距 4/3pt），order 标题不重复输出（`suppressTitle` 含 order/minutes，红标即标题）。命令文号"第×号"居中排于标志下。
- **上行文缺文号时签发人丢失（批量审计发现）**：`toMarkdown` 原文号分支 `if (doc.docNumber)` 才渲染文号+签发人容器，请求/报告缺文号（请示可无文号、仅靠签发人标识上行文）时整个容器被跳过、签发人静默丢弃。修复：分支条件改为 `doc.docNumber || (上行文 && signer)`——有签发人时即使无文号也渲染 `.doc-number-line` flex（签发人"签发人：×××"居右空一字）。
- **发文字号顺序号误加"第"字 / 缺"号"字（GB/T 9704 §7.2.5）**：顺序号前不应加"第"（"国发〔2026〕第12号"非法）、后必须加"号"（"国发〔2026〕12"非法）。此前只校验了前导 0，这两类违规放行。修复：`doc-type-spec` 新增 `DOC_NUMBER_SEQUENCE_MALFORMED` 通用规则（两个定向正则增量检测，不校验整串格式，避免误伤机关代字含字母/占位符 × 的合法形态）；命令（令）顺序号"第×号"由 ORDER_DOC_NUMBER_SHOULD_BE_SEQ 单独约束，不适用。配套：prompt 增加 §7.2.5 文号格式指令（年份六角括号全称、顺序号不加"第"不编虚位加"号"字）。
