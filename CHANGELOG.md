...
- **默认规则（GB/T 33476-2016）红色分隔线失效，`---` 渲染成多余短横线"—"**（文秘验收问题 3/4 根因）：红色分隔线实现时只修了 gb-t-9704.yaml，但应用默认规则是 `builtin[0]` = **GB/T 33476-2016**，其 `disabledSyntax` 仍含 `horizontalRule` → markdown `---` 被 typographer 降级成字面 em-dash"—"，红线不渲染、标题间距逻辑随之失效。修复：gb-t-33476.yaml 的 `disabledSyntax` 移除 `horizontalRule`（与 9704、parser 默认一致）。持久化规则与 builtin 同名时会从新 builtin 刷新（rule-store 已支持的机制），存量用户重启即修复。
