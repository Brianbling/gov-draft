...
describe("validateRule", () => {
  ...
  it("returns invalid when pagination format contains illegal expression", () => {
    ...
    expect(result.valid).toBe(false)
    expect(result.errors).toEqual(
      expect.arrayContaining(["pagination.format: INVALID_EXPRESSION"])
    )
  })
  ...
  it("returns valid when pagination hideFirstPage is a boolean (首页不编页码)", () => {
    const validRule = createValidRule()
    if (!validRule.pagination) {
      throw new Error("pagination 配置缺失")
    }
    validRule.pagination.hideFirstPage = true
    const result = validateRule(validRule)
    expect(result.valid).toBe(true)
  })

  it("returns invalid when pagination hideFirstPage is not a boolean", () => {
    const invalidRule = createValidRule()
    if (!invalidRule.pagination) {
      throw new Error("pagination 配置缺失")
    }
    invalidRule.pagination.hideFirstPage = "yes" as unknown as boolean
    const result = validateRule(invalidRule)
    expect(result.valid).toBe(false)
    expect(result.errors).toContain("pagination.hideFirstPage: BOOLEAN")
  })
  ...
  it("returns invalid when page margins are not convertible to px", () => {
    ...
    const result = validateRule(invalidRule)
    expect(result.valid).toBe(false)
    expect(result.errors).toContain("page.margins.top: CONVERTIBLE_CSS_LENGTH")
  })

  it("returns valid when content contains custom level with full ContentItem structure", () => {
    const validRule = createValidRule()
    validRule.content.appendix = JSON.parse(
      JSON.stringify(validRule.content.body)
    )
