...
const PAGINATION_HORIZONTAL_ANCHOR_SET = new Set([
  "left",
  "center",
  "right",
  "outside",
...
const PAGINATION_NUMBER_STYLE_SET = new Set([
  "arabic",
  "roman",
  "zhHans",
  "zhHant",
...
const validateCssLineHeight = patternValidator(
  CSS_LINE_HEIGHT_PATTERN,
  ValidationErrorCode.CSS_LINE_HEIGHT
)

const validateCssColor = patternValidator(
  CSS_COLOR_PATTERN,
  ValidationErrorCode.CSS_COLOR
)

const validateFontWeight = setValidator(
  FONT_WEIGHT_SET,
  ValidationErrorCode.FONT_WEIGHT
)

const validateTextAlign = setValidator(
  ALIGN_SET,
  ValidationErrorCode.TEXT_ALIGN
)

const validateBoolean = typeValidator("boolean", ValidationErrorCode.BOOLEAN)

const validateEnterStyle = setValidator(
  ENTER_STYLE_SET,
  ValidationErrorCode.ENTER_STYLE
)
...
const validateCssParagraphSpacing: Validator = (value, path, issues) => {
  ...
  if (
    typeof value !== "string" ||
    !CSS_PARAGRAPH_SPACING_PATTERN.test(value.trim())
  ) {
...
function validateContentItem(
  ...
  if (!isObject(contentItem.fonts)) {
    pushError(
      issues,
      `${path}.fonts`,
      ValidationErrorCode.MISSING_OR_INVALID_FIELD
    )
  } else {
    validateString(
      contentItem.fonts.latinFamily,
      `${path}.fonts.latinFamily`,
      issues
    )
    validateString(
      contentItem.fonts.cjkFamily,
      `${path}.fonts.cjkFamily`,
      issues
    )
    if (contentItem.fonts.cnQuoteFamily !== undefined) {
      validateString(
        contentItem.fonts.cnQuoteFamily,
        `${path}.fonts.cnQuoteFamily`,
        issues
      )
    }
    if (contentItem.fonts.cnBookTitleFamily !== undefined) {
      validateString(
        contentItem.fonts.cnBookTitleFamily,
        `${path}.fonts.cnBookTitleFamily`,
        issues
      )
    }
  }
  ...
  if (!isObject(contentItem.style)) {
    ...
    if (!isObject(contentItem.style.colors)) {
      ...
      validateCssColor(
        contentItem.style.colors.text,
        `${path}.style.colors.text`,
        issues
      )
      validateCssColor(
        contentItem.style.colors.background,
        `${path}.style.colors.background`,
        issues
      )
    }

    if (
      contentItem.style.index !== undefined &&
      contentItem.style.index !== null &&
      typeof contentItem.style.index !== "string"
    ) {
  ...
  if (!isObject(contentItem.paragraph)) {
    ...
    validateTextAlign(
      contentItem.paragraph.align,
      `${path}.paragraph.align`,
      issues
    )
    validateCssLength(
      contentItem.paragraph.indent,
      `${path}.paragraph.indent`,
      issues
    )
    validateCharsPerLine(
      contentItem.paragraph.charsPerLine,
      `${path}.paragraph.charsPerLine`,
      issues
    )
    ...
    if (!isObject(contentItem.paragraph.spacing)) {
      ...
      validateCssLineHeight(
        contentItem.paragraph.spacing.lineHeight,
        `${path}.paragraph.spacing.lineHeight`,
        issues
      )
      validateCssParagraphSpacing(
        contentItem.paragraph.spacing.before,
        `${path}.paragraph.spacing.before`,
        issues
      )
      validateCssParagraphSpacing(
        contentItem.paragraph.spacing.after,
...
function validatePage(
  ...
  const hasNamedSize =
    typeof page.size === "string" && page.size.trim().length > 0
  const hasCustomDimensions = isObject(page.dimensions)
  if (!hasNamedSize && !hasCustomDimensions) {
    pushError(
      issues,
      `${path}.size`,
      ValidationErrorCode.PAGE_SIZE_OR_DIMENSIONS
    )
  }
  ...
  if (page.orientation !== undefined) {
    if (
      typeof page.orientation !== "string" ||
      !PAGE_ORIENTATION_SET.has(page.orientation)
    ) {
      pushError(
        issues,
        `${path}.orientation`,
        ValidationErrorCode.PAGE_ORIENTATION
      )
    }
  }
...
function validatePaginationConfig(
  pagination: unknown,
  path: string,
  issues: ValidationIssue[]
): void {
  ...
  if (pagination.numberStyle !== undefined) {
    if (
      typeof pagination.numberStyle !== "string" ||
      !PAGINATION_NUMBER_STYLE_SET.has(pagination.numberStyle)
    ) {
      pushError(
        issues,
        `${path}.numberStyle`,
        ValidationErrorCode.PAGINATION_NUMBER_STYLE
      )
    }
  }

  if (pagination.hideFirstPage !== undefined) {
    validateBoolean(pagination.hideFirstPage, `${path}.hideFirstPage`, issues)
  }
...
function validatePaginationStyle(
  style: AnyRecord,
  path: string,
  issues: ValidationIssue[]
): void {
...
function validatePaginationPosition(
  position: AnyRecord,
  path: string,
  issues: ValidationIssue[]
): void {
  if (!isObject(position.vertical)) {
    ...
    if (
      typeof position.vertical.anchor !== "string" ||
      !PAGINATION_VERTICAL_ANCHOR_SET.has(position.vertical.anchor)
    ) {
      pushError(
        issues,
        `${path}.vertical.anchor`,
        ValidationErrorCode.VERTICAL_ANCHOR
      )
    }
    validateCssLength(
  ...
  if (!isObject(position.horizontal)) {
    ...
    if (
      typeof position.horizontal.anchor !== "string" ||
      !PAGINATION_HORIZONTAL_ANCHOR_SET.has(position.horizontal.anchor)
    ) {
      pushError(
        issues,
        `${path}.horizontal.anchor`,
        ValidationErrorCode.HORIZONTAL_ANCHOR
      )
    }
    validateCssLength(
...
function validatePaginationFormat(
  value: unknown,
  path: string,
  issues: ValidationIssue[]
): void {
  ...
  for (const match of expressionMatches) {
    ...
    if (!isValidPaginationExpression(expression)) {
      pushError(issues, path, ValidationErrorCode.INVALID_EXPRESSION, {
...
function validateConvertiblePageMargin(
  value: unknown,
  path: string,
  issues: ValidationIssue[]
): void {
  ...
  if (!Array.isArray(parser.disabledSyntax)) {
    ...
    parser.disabledSyntax.forEach((item, index) => {
      if (typeof item !== "string" || item.trim().length === 0) {
        pushError(
          issues,
          `${path}.disabledSyntax.${index}`,
          ValidationErrorCode.INVALID_SYNTAX_ITEM
        )
      }
    })
  }
    ...
    if (!isObject(parser.localStyleAliases)) {
      ...
      Object.entries(parser.localStyleAliases).forEach(([alias, target]) => {
        if (alias.trim().length === 0) {
          pushError(
            issues,
            `${path}.localStyleAliases`,
            ValidationErrorCode.ALIAS_KEY_EMPTY
          )
          return
        }
        ...
        if (typeof target !== "string") {
          pushError(
            issues,
            `${path}.localStyleAliases.${alias}`,
            ValidationErrorCode.ALIAS_TARGET_STRING
          )
          return
        }
        ...
        if (!pathValidation.formatValid) {
          pushError(
            issues,
            `${path}.localStyleAliases.${alias}`,
            ValidationErrorCode.ALIAS_TARGET_FORMAT
          )
          return
        }
        ...
        if (!pathValidation.inScope) {
          pushError(
            issues,
            `${path}.localStyleAliases.${alias}`,
            ValidationErrorCode.ALIAS_TARGET_SCOPE
          )
          return
        }
        ...
        if (!pathValidation.safe) {
          pushError(
            issues,
            `${path}.localStyleAliases.${alias}`,
            ValidationErrorCode.ALIAS_TARGET_UNSAFE
          )
        }
      })
    }
